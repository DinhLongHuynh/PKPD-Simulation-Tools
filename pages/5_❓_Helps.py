...
st.write('This model helps to visualize the PK profile using a one compartmental model. In this model, the central compartment is the only site of absorption and elimination.')
...
st.write('- **Absorption Rate Constant (h-1):** The data type is **float**. This is the rate constant that characterizes the absorption, ka. If the dosing regimen has at least 1 non-iv dose, the ka is mandatory. Otherwise, this value can be left as None.')
...
st.write('- **Dose Amount (mg):** The data type is **float**. This is dosage used for the simulation.')
...
st.write("- **C Limit (mg/L):** The data type is **float**. This is the concentration that you don't want the drug to exceed, i.e. maximum tolerated dose. The value can be a blank or a float number.")
st.write('\n')
...
st.write('- **Population Absorption Constant (h-1):** The data type is **float**. This is the mean of ka among the whole population. In case you want to simulate an iv drug, the value of ka can be blank, which means ignoring the absorption process.')
...
st.write("- **Compulsory Information**: These are the fundamental data that one clinical study must have. If your dataset doesn't have enough compulsory information, you should double check your data, or your study design. ")
st.write("- **Additional Information**: These are optional. If they are not available, you can leave it blank.")
st.write("\n")
st.write("\n")

st.subheader('Non-Compartmental Analysis')
st.write("Non-compartmental analysis (NCA) is a method used in pharmacokinetics to analyze and interpret drug concentration data without assuming a specific compartmental model for the drug distribution and elimination processes. Instead of relying on a predetermined biological model, NCA calculates PK parameters directly from the observed concentration-time data.")
